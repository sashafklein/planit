class Place < ActiveRecord::Base
  ...
  scope :by_ll_and_name, -> (atts, name) { where.not( lat: nil ).where.not( lon: nil ).by_ll(atts[:lat], atts[:lon]).with_name(name) }
  scope :by_location, -> (atts) { atts[:street_address].present? ? with_nonempty_street_address.with_region_info(atts).with_address(atts[:street_addresses]) : none }
