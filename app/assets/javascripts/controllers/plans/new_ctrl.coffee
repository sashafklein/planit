...
# GOOGLE API WORK
  ...
  $s._getLatLon = (locale) ->
    $http.get("http://nominatim.openstreetmap.org/search?q=#{locale}&format=json")

  $s._getGoVenue = (destination, locale) ->
    latlon_item = $s._getLatLon(locale)
      .success (response) ->
        alert("#{$s._baseGoPath}query=#{destination}&location=#{response[0].lat},#{response[0].lon}&radius=500&key=#{$s._goAuth}")
