...
mod.factory 'PlaceFilterer', ($filter) ->
  ...
  class PlaceFilterer
    ...
    returnFiltered: (places) -> 
      ...
      $filter('filter')( places, ( (place) => @_filterPlace(place) ) )

    _filterPlace: (place) ->
      [context, filters] = [@, [ '_metaCategoryFilter' ]] # @_been, @_loved, @_open, '_queryFilter', '_wifiFilter'
      ...
      if newObj.f
        for val in newObj.f.split(",")
          if val == "seedo"
            ...
            newObj.meta_categories.push("Area", "Help", "Money", "Other", "Transit", null)
          else if val == 'wifi'
            newObj.wifi = true    
          else
