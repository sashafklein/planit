angular.module("Common").directive 'itemEntryForm', (User, ErrorReporter, CurrentUser, Plan, Foursquare, Item, Place, $filter) ->
  return {
    restrict: 'E'
    ...
    link: (s, e, a) ->
      ...
      s.listOptions = ->
        ...
        return [] unless s.visibleLists?.length
        filter(s.visibleLists, s.listQuery)

      s.noListOptions = ->
        !s.listOptions().length && !s.list?
