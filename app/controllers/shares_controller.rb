class SharesController < ApplicationController
  ...
  def create
    if is_email?( share_params[:email] )
      ...
      if Share.save_and_send(sharee: sharee, sharer: current_user, url: request.env["HTTP_REFERER"], object: Share.find_object(request.env["HTTP_REFERER"]), notes: share_params[:notes])
