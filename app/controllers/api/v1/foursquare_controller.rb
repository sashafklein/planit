class Api::V1::FoursquareController < ApiController
  ...
  def search
    ...
    response = client.explore_venues( near: params[:near], query: params[:query] ).to_sh.super_fetch(:groups, 0, :items)
    
    render json: (response ? response.map{ |v| Completers::ApiVenue::FoursquareExploreVenue.new(v).serialize([:icon]) } : [])
  end
