...
describe "All views" do
  ...
  def run_tests(role:, context: nil, owns_objects: false)
    ...
    RouterHelper.get_routes_by_controller( %w(mini_scrape oauths allowable bookmarklets search point unmatched_route morph find_all_notes_in_plan find_by_object located_near /admin/ geonames) ).each do |ctrl_group|
